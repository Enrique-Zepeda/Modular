...
export default function ProfileTopExercises({ username, topN = 3 }: { username: string; topN?: number }) {
  ...
  return (
    <Card className="w-full border-2 border-border/60 bg-gradient-to-br from-card via-card/98 to-card/95 shadow-lg hover:shadow-xl hover:shadow-primary/10 hover:-translate-y-1 transition-all duration-300 overflow-hidden relative">
      ...
      <div className="absolute top-0 right-0 w-32 sm:w-48 h-32 sm:h-48 bg-primary/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
      ...
      <CardContent className="relative p-4 sm:p-6">
        <div className="mb-4 sm:mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="inline-flex items-center gap-2 text-[0.7rem] sm:text-xs font-bold text-primary bg-primary/10 px-3 py-1.5 rounded-lg border border-primary/30 w-fit">
        ...
        <div className="space-y-4 sm:space-y-5">
          {items.map((ex, idx) => {
            ...
            return (
              <div key={ex.id} className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-5">
                <div className="relative h-24 w-24 mx-auto sm:mx-0 rounded-xl overflow-hidden border-2 border-primary/40 ring-4 ring-primary/15 shadow-xl shrink-0 flex items-center justify-center bg-gradient-to-br from-primary/25 via-primary/15 to-purple-500/15 hover:scale-105 transition-transform duration-300">
                  {ex.ejemplo ? (
                    <img
                  ...
                  <div className="absolute top-1.5 right-1.5 bg-primary text-primary-foreground text-[0.7rem] font-bold px-2 py-1 rounded-md shadow-lg">
                    #{idx + 1}
                  </div>
                </div>

                <div className="min-w-0 flex-1 space-y-3">
                  <div className="text-base sm:text-lg font-extrabold leading-tight line-clamp-2">{ex.nombre}</div>
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 text-xs sm:text-sm">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/20 border-2 border-blue-500/50 rounded-lg shadow-sm">
                      ...
                      <span className="text-blue-600/80 dark:text-blue-300/80 text-[0.7rem] sm:text-xs font-medium">
                        sets
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/20 border-2 border-emerald-500/50 rounded-lg shadow-sm">
                      ...
                      <span className="text-emerald-600/80 dark:text-emerald-300/80 text-[0.7rem] sm:text-xs font-medium">
                        sesiones
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/20 border-2 border-purple-500/50 rounded-lg shadow-sm">
                      ...
                      <span className="text-purple-600/80 dark:text-purple-300/80 text-[0.7rem] sm:text-xs font-medium">
                        {unit}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="pt-1.5 sm:pt-2">
                    <div className="mb-1.5 sm:mb-2 flex items-center justify-between text-[0.7rem] sm:text-xs text-muted-foreground">
