...
type Props = {
  ...
  /** ← SIEMPRE llega un string (p. ej. "Fácil", "Sin sensaciones") */
...
export function WorkoutCard({
  ...
  // tono del badge según sensación (leve estética)
  const badgeVariant = useMemo<"secondary" | "default" | "outline">(() => {
    const v = sensacionFinal.toLowerCase();
    if (v.includes("fácil")) return "secondary";
    if (v.includes("moderado")) return "secondary";
    if (v.includes("difícil")) return "default";
    if (v.includes("fallo")) return "default";
    return "outline";
  }, [sensacionFinal]);
        ...
        <CardHeader className="relative pb-5 z-10">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              {/* ← SIEMPRE se muestra, ya que nos llega un string garantizado */}
              <Badge variant={badgeVariant} className="rounded-2xl px-4 py-2 text-xs font-semibold">
                {sensacionFinal}
              </Badge>
            </div>
          </div>
          ...
          {ejercicios && ejercicios.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2">
              {ejercicios.map((ex, idx) => (
                <div key={(ex.id ?? idx)?.toString()} className="rounded-2xl border border-border/30 bg-muted/20 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        {ex.sets_done && <span className="font-semibold text-foreground/80">{ex.sets_done} sets</span>}
