...
export default function FriendsPage() {
  ...
  return (
    <motion.div
      ...
      className="mx-auto max-w-[min(100%,theme(spacing.7xl))] px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8"
    >
      {/* Hero / encabezado: mobile-first, apila en columna y luego pasa a fila */}
      <motion.div
        initial={{ opacity: 0, y: -30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.2, duration: 0.6 }}
        className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/10 via-accent/5 to-transparent p-6 sm:p-8 lg:p-10 border-2 border-border/60 backdrop-blur-xl shadow-xl md:shadow-2xl shadow-primary/10"
      >
        ...
        <div className="absolute bottom-0 left-0 w-56 sm:w-64 h-56 sm:h-64 bg-accent/5 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 pointer-events-none" />

        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
          <div className="relative mx-auto sm:mx-0 flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-primary/30 to-accent/20 ring-2 ring-primary/30 shadow-2xl shadow-primary/30">
            <Users className="h-8 w-8 sm:h-10 sm:w-10 text-primary drop-shadow-lg" />
          ...
          <div className="flex-1 min-w-0 text-center sm:text-left">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight bg-gradient-to-r from-foreground via-primary/90 to-accent/90 bg-clip-text text-transparent drop-shadow-sm text-balance">
              Amigos
            </h1>
            <p className="text-muted-foreground mt-2 sm:mt-3 text-sm sm:text-base lg:text-lg leading-relaxed font-medium text-pretty">
      ...
      {/* Buscador centrado y full-width en mobile */}
      <motion.div
      ...
      >
        <div className="relative group">
          ...
          <Input
            ...
            className="pl-12 h-12 sm:h-14 text-base border-2 border-border/60 focus-visible:border-primary/50 focus-visible:ring-4 focus-visible:ring-primary/10 transition-all duration-200 bg-card/50 backdrop-blur-sm shadow-lg md:hover:shadow-xl rounded-xl"
            inputMode="search"
            spellCheck={false}
          />
        </div>
              ...
              <div className="p-8 sm:p-10 lg:p-12 text-center space-y-6">
                <div className="flex justify-center">
                  <div className="relative">
                    ...
                    <div className="relative flex h-20 w-20 sm:h-24 sm:w-24 items-center justify-center rounded-full bg-gradient-to-br from-muted/50 to-muted/30 ring-2 ring-border/50">
                      {debounced.length < MIN_CHARS ? (
                        <UserPlus className="h-10 w-10 sm:h-12 sm:w-12 text-muted-foreground/70" />
                      ) : (
                        <Search className="h-10 w-10 sm:h-12 sm:w-12 text-muted-foreground/70" />
                      )}
                    </div>
                  </div>
                </div>
                ...
                <AnimatePresence initial={false}>
                  {results.map((u: any, idx: number) => {
                    ...
                    return (
                      ...
                      >
                        <div className="group px-4 sm:px-6 py-4 sm:py-5 md:hover:bg-gradient-to-r md:hover:from-primary/5 md:hover:via-accent/5 md:hover:to-transparent transition-all duration-300 relative">
                          <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-primary to-accent opacity-0 md:group-hover:opacity-100 transition-opacity duration-300" />

                          {/* En mobile apila avatar+texto y la acción debajo; en desktop quedan en fila */}
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                            <Link
                              to={`/u/${u.username}`}
                              className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1 group/link"
                            >
                              ...
                              <div className="min-w-0 space-y-0.5">
                                <div className="font-bold text-sm sm:text-base truncate md:group-hover/link:text-primary transition-colors duration-200">
                                  {u.nombre ?? `@${u.username}`}
                                </div>
                                <div className="text-xs sm:text-sm text-muted-foreground truncate font-medium">
                                  @{u.username}
                                </div>
                              </div>
                            </Link>
                            ...
                            <FriendshipBadge
                              ...
                              className={cn(
                                // Full-width en mobile para que no genere scroll, compacto en desktop
                                "mt-2 w-full sm:mt-0 sm:w-auto sm:ml-auto flex-shrink-0 shadow-sm md:hover:shadow-md transition-shadow duration-200"
                              )}
                            />
                          </div>
                        </div>
