...
export async function debugAuthStatus() {
  try {
    ...
    if (session?.user) {
      ...
      const { data: rutinasTest, error: rutinasError } = await supabase.from("Rutinas").select("count").limit(1);
...
export async function debugCreateRutina() {
  try {
    ...
    const { data, error } = await supabase.from("Rutinas").insert([testPayload]).select().single();
